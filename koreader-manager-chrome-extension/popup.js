    ...
    if (tab && tab.url) {
      ...
      chrome.downloads.download(
        ...
        downloadId => {
          ...
          showBookmarkStatus('✓ Current tab URL saved to urls.csv');
          showNotification(1);
        },
      );
    }
  } catch (error) {
    console.error('Error exporting current tab:', error);
  }
});
...
function showKOReaderStatus(message, type) {
...
async function scanNetwork() {
  ...
  if (!prefix) {
    showKOReaderStatus('🔍 Detecting network...', 'scanning');
    prefix = await detectNetworkPrefix();
    networkPrefixInput.value = prefix;
  }

  showKOReaderStatus(`🔍 Scanning ${prefix}.x network...`, 'scanning');
  ...
  for (const lastOctet of priorityIps) {
    ...
    koreaderStatusEl.innerHTML = `<span class="spinner"></span>Checking ${ip}...`;
  ...
  if (!found) {
    koreaderStatusEl.innerHTML = `<span class="spinner"></span>Full scan in progress...`;
    ...
    for (let i = 2; i <= 253; i++) {
      ...
      if (i % 10 === 0) {
        ...
        koreaderStatusEl.innerHTML = `<span class="spinner"></span>${progress}% (${ip})`;
      }
  ...
  if (!found) {
    showKOReaderStatus(
      '❌ Kindle not found. Try manual IP or check if File Browser is running.',
      'error',
    );
  }
...
function manualIpEntry() {
  ...
  if (ip) {
    showKOReaderStatus(`🔍 Checking ${ip}...`, 'scanning');
